...
def expenses(post: schemas.PostCreate, db: Session = Depends(get_db) ):
    ...
    new_row = models.Expense(**post.model_dump())
    db.add(new_row)
    db.commit()
    db.refresh(new_row)
...
def update(id: int, data: schemas.UserUpdate, db: Session = Depends(get_db)):

    row_query = db.query(models.Expense).filter(models.Expense.id == id)
    row = row_query.first()
    ...
    row_query.update(update_data, synchronize_session=False)
    db.commit()
    updated_row = row_query.first()
...
def get_expenses( month: Optional[str] = "", db:Session = Depends(get_db) ):
    
    try:
        if month == "":
            rows_query = db.query(models.Expense)   
        else:
            int_month = monthdict[month]
            rows_query = db.query(models.Expense).filter(extract('month', models.Expense.created_at) == int_month)
...
def get_expenses(year: Optional[int] = None, db:Session = Depends(get_db)):
    ...
    rows_query =db.query(models.Expense).filter(extract('year', models.Expense.created_at) == year)
    rows = rows_query.all()
